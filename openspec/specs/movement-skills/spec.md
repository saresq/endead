# Movement Skills Specification

## Purpose

Make the skills that move a survivor — Sprint, Charge, Bloodlust and Born Leader — obey the ordinary movement rules along every zone they cross, and follow what their cards actually say.
...
### Requirement: Charge and Bloodlust respect zombies along the path
Charge and Bloodlust SHALL apply normal movement rules to every zone they pass through, including the middle zone of a two-zone path. Both read "move up to 2 Zones to a Zone with at least 1 Zombie", so they follow the same rule.
...
#### Scenario: A clear middle zone
- **WHEN** the middle zone holds no zombies
- **THEN** the survivor reaches the destination
