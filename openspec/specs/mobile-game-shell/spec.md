# Mobile Game Shell Specification

## Purpose

Define the in-game shell across viewports: the portrait bottom sheet and its snap points, the landscape side panel, the tablet threshold into the desktop layout, dynamic viewport units and safe areas, and touch target sizing.
...
### Requirement: Tablet uses the desktop layout
Viewports at least 744px wide and at least 600px tall SHALL use the desktop layout, in either orientation. No orientation test is needed: a portrait viewport at least 744px wide is necessarily at least 744px tall, so it cannot fall below the height threshold. Below that threshold the game SHALL use the landscape phone layout when wider than tall, and the portrait sheet otherwise.

The desktop layout's own arrangement — a bottom deck rather than the right rail this change originally shipped — is specified by `desktop-board-deck`, which replaced the rail and raised the height threshold from 500px to 600px.

#### Scenario: iPad Mini landscape
- **WHEN** the game is shown at 1024x744 or 1133x744
- **THEN** the desktop layout is used and the board fills the height above the deck

#### Scenario: iPad portrait
- **WHEN** the game is shown at 744x1133
- **THEN** the desktop layout is used, not the phone sheet

#### Scenario: Wide landscape phone
- **WHEN** the game is shown at 844x390
- **THEN** the landscape phone layout with the right side panel is used, not the desktop layout

#### Scenario: Short landscape viewport
- **WHEN** the game is shown at 744x599
- **THEN** the landscape phone layout with the right side panel is used
