...
- [x] 6.2 Change the rail breakpoint to `(min-width: 744px) and (min-height: 500px) and (orientation: landscape)`, defined once in `src/client/ui/layoutQueries.ts`; `GameHUD` stamps `data-layout` on `#game-hud` and `hud.css` styles by it (replaces `laptopBreakpoint`)
